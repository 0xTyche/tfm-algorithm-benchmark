...
% Add Bio-Formats MATLAB wrappers (bfmatlab). Required for bfGetReader/bfopen.
addpath(genpath([pwd filesep 'bfmatlab']));

% Ensure Bio-Formats Java library is available for reading microscopy formats (e.g. .lif).
% bfGetReader() depends on Bio-Formats classes (loci.formats.*) being on the Java classpath.
try
    bfJar = fullfile(pwd, 'bfmatlab', 'bioformats_package.jar');
    if exist(bfJar, 'file') == 2
        % Add to the *dynamic* Java classpath for this MATLAB session.
        dynJcp = javaclasspath('-dynamic');
        if ~any(strcmpi(dynJcp, bfJar))
            javaaddpath(bfJar);
        end
    end
catch
    % If this fails, bfGetReader will throw a clear error later.
end
