...
% NOTE: Some distributions place elastix binaries in /elastixLib, others in /external/elastixLib.
% We auto-detect the correct location.
elastix_candidate_1 = [path_names.installation_path filesep 'elastixLib'];
elastix_candidate_2 = [path_names.installation_path filesep 'external' filesep 'elastixLib'];
if isfolder(elastix_candidate_2)
    path_names.elastixLibFolder = elastix_candidate_2;
elseif isfolder(elastix_candidate_1)
    path_names.elastixLibFolder = elastix_candidate_1;
else
    % Fallback to the original default (user can override manually below)
    path_names.elastixLibFolder = elastix_candidate_2;
    warning('TFMLAB:ElastixNotFound', ...
        'Elastix folder not found. Expected "%s" or "%s". Displacement calculation will fail until fixed.', ...
        elastix_candidate_1, elastix_candidate_2);
end
