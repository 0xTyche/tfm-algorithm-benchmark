...
function [replaceStrIn,replaceStrOut,deleteLineKeyword] = parseElastixDispCalcParam(options)
...
if isempty(options.optim.evalSampleNum)
    ...
    if options.mask.flag
        replaceStrIn{strCount} = 'imageSamplerVal'; replaceStrOut{strCount} = 'RandomSparseMask'; strCount = strCount+1;
    else
        replaceStrIn{strCount} = 'imageSamplerVal'; replaceStrOut{strCount} = 'RandomCoordinate'; strCount = strCount+1;
    end
    replaceStrIn{strCount} = 'sampleNumVal'; replaceStrOut{strCount} = numMtx2Str(options.optim.evalSampleNum); strCount = strCount+1; % for each scale
end
replaceStrIn{strCount} = 'outputImType';  replaceStrOut{strCount} = options.outImType; strCount = strCount+1;

replaceStrIn{strCount} = 'xyGridMin';  replaceStrOut{strCount} = num2str(options.grid.minSpacing.xy); strCount = strCount+1;
switch options.imDim
    case 2
        replaceStrIn{strCount} = 'zGridMin';  replaceStrOut{strCount} = ''; strCount = strCount+1;
    case 3
        replaceStrIn{strCount} = 'zGridMin';  replaceStrOut{strCount} = num2str(options.grid.minSpacing.z); strCount = strCount+1;
end
replaceStrIn{strCount} = 'gridSpacingScheduleVal';  replaceStrOut{strCount} = numMtx2Str(options.grid.spacingSchedule); strCount = strCount+1; % for each scale
replaceStrIn{strCount} = 'passiveEdgeGridPointNum'; replaceStrOut{strCount} = numMtx2Str(options.grid.passiveEdgeGridPointNum); strCount = strCount+1; % for each scale
