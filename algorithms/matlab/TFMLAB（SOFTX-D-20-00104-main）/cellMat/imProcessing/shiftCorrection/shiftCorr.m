...
switch type
    case 'bin'
        % Explicit boundary condition (DIPimage): pad outside image with zeros
        im = resample(dip_image(im)>0,1,shft,'nn','add zeros');
    case {'field','im'}
        % Explicit boundary condition (DIPimage): pad outside image with zeros
        im = resample(dip_image(im),1,shft,'bspline','add zeros');
end
...
% uses explicit boundary_condition arguments (see resample()).
